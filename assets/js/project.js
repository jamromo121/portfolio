export const project = [
  // {
  //   id: 9,
  //   title: "SPOTTA NG",
  //   description:
  //     "See through the <span>lenses of people</span> who have lived or visited the <span>neighbourhood you might have in mind.</span>",
  //   category: "Design,",
  //   others: "Figma",
  //   img: "./assets/img/work9.png",
  //   link: "https://bit.ly/3U2lMUN",
  // },
  {
    id: 10,
    title: "STI College Ormoc Enrollment System",
    description:
      "Simplifying Student Registration with <span>Efficiency</span> and <span>Accuracy.</span>",
    category: "C# , .NET",
    others: "",
    img: "./assets/img/work9.png",
    
  },
  {
    id: 9,
    title: "STI Connect STI College Ormoc Student Engagement and Learning System",
    description:
      "Enhancing  <span> Student Engagement </span> and  <span> Activity Management <span> with <span> Smart Digital Solutions </span>.",
    category: "",
    others: "SQL, C#, .NET",
    img: "./assets/img/work1.png",
    link: "https://nakamurafc.com/",
  },
  {
    id: 8,
    title: "KOZO Earning App",
    description:
      "Maximize Earnings <span> With Low Risk </span>and <span>High Rewards</span>.",
    category: "Design, Web App",
    others: "Adobe XD, HTML, CSS, PHP, JAVAScript",
    img: "./assets/img/work7.png",
    link: "https://www.figma.com/file/QdGMxn0FQWA5hDriMLdoSl/OptimuzHomez?type=design&node-id=0%3A1&mode=design&t=UWWMa35vNYAkS57A-1",
  },
  {
    id: 7,
    title: "Parkfy Traffic and Parking Management System",
    description:
      "Reliable, Efficient, <span>Parking and Traffic Monitoring </span> Solutions</span>.",
    category: "WebApp",
    others: "PHP, JavaScript, HTML, CSS",
    img: "./assets/img/work10.png",
    link: "https://pvgnl.com/",
  },
  {
    id: 6,
    title: "CashWise Bank App",
    description:
      "A prototype bank app <span> for the visual impaired. </span> it features <span>Text to speech for accessibility </span>.",
    category: "Design,",
    others: "Adobe XD",
    img: "./assets/img/work8.png",
    link: "https://xd.adobe.com/view/948b0329-3878-4714-9de8-2ef0b99ee092-c1d8/",
  },
  {
    id: 5,
    title: "E-waste Management",
    description:
      "An e-waste management app<span> for recycle waste around Lagos </span> with ease.<span> It utilizes geolocation to pinpoint waste collection points.</span>.",
    category: "Design,",
    others: "Figma",
    img: "./assets/img/work6.png",
    link: "https://www.figma.com/file/v7j37pvIsdXi8D4mo3sKWe/E-Waste-Management?type=design&node-id=18%3A112&mode=design&t=OndY7poaBzVae13M-1",
  },
  {
    id: 4,
    title: "Travelia Agency",
    description:
      "Landing Page for Travelia <span>Travel Agency</span> Service with <span>Clean UI and Interactive Design</span>",
    category: "JavaScript,",
    others: "HTML, CSS",
    img: "./assets/img/work2.png",
    link: "https://traveliaagency.netlify.app/",
  },
  {
    id: 3,
    title: "Firebase Task Manager",
    description:
      "Task Manager With <span>Firebase Authentication</span> And <span>Firestore</span> CDN - JS ES6, HTML and CSS",
    category: "Javascript,",
    others: "HTML, CSS",
    img: "./assets/img/work3.png",
    link: "https://lovely-madeleine-ab3068.netlify.app/",
  },
  {
    id: 2,
    title: "Weather App",
    description:
      " A Weather App with the <span>Open Weather API</span> to display current <span>weather information</span> based on location</span>",
    category: "JavaScript,",
    others: "HTML, CSS",
    img: "./assets/img/work4.png",
    link: "https://cheery-madeleine-a3db79.netlify.app/",
  },
  {
    id: 1,
    title: "Form Validation",
    description:
      "A Form Validation built with <span>JavaScript and Bootstrap</span> to Validate <span>Email Address</span> and input data",
    category: "JavaScript,",
    others: "Bootstrap 5, CSS",
    img: "./assets/img/work5.png",
    link: "https://keen-bavarois-0eb8c4.netlify.app/",
  },
];
